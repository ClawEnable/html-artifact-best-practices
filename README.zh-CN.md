...
创建新 artifact：
> "为这些数据库方案创建一个对比矩阵，并保存为 database-comparison.html。"

审查已有 artifact：
> "根据 html-artifact-guide 检查清单审查这个 HTML artifact。"

改进 artifact：
> "改进这个 artifact 的移动端可读性、打印输出和可复制性。"

当 HTML 没有必要时使用 Markdown：
> "总结这些会议纪要。除非 HTML 明确提升审阅价值，否则使用 Markdown。"

生成的 artifact 应是独立 `.html` 文件，使用内联 CSS、原生 JS、无 CDN，并包含 AI 验证提示。
...
- **通过验证器的独立模板** — 用于新 artifact 的响应式、focus-visible、打印就绪基线
- **触发 Eval 场景** — 20 个结构化用例，覆盖 HTML 生成、Markdown 回退和澄清边界
- **Artifact 验证器** — 零依赖 Node.js 发布门禁，检查元数据、依赖策略、焦点、打印、可复制性和移动端风险

## 安装

已验证的安装路径是 Claude Code 和 Codex CLI。其他兼容 Agent Skills 的环境可能可用，但本仓库目前未验证。
...
仅在当前 Claude Code 会话中加载这个 checkout：
...
更新时，重新执行 `npx skills add ClawEnable/html-artifact-best-practices`，或对 npx 管理的安装执行 `npx skills update html-artifact-guide`。

</details>

<details>
<summary><strong>其他 Agent</strong> — 未验证</summary>

可分发的 Skill 位于 `skills/html-artifact-guide/`。支持 Agent Skills 目录格式的环境可能可以导入该目录，但本仓库目前只验证 Claude Code 和 Codex CLI 安装路径。
